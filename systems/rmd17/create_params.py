...
DEFAULT_PARAMS = {
    ...
    "dft": {
        ...
        "parentconfig": "nn_evidential_attack",
        "method": "nn_evidential_attack",
        "method_description": "from addxyz",
        "childconfig": "bp86_d3_def2svp_engrad_orca",
        "childconfig_method": "dft_d3_gga_bp86",
        "childconfig_method_description": "Orca bp86/def2-SVP D3 DFT RI",
    },
}
...
def add_args():
    ...
    parser.add_argument("molecule", type=str, help="Molecule to train on")
...
def check_input_args(args):
    if args.model__uncertainty_type == "ensemble":
        ...
        if args.model__num_networks < 2:
            args.model__num_networks = 5
            warnings.warn("Ensemble model only has one network, changed to 5")
    ...
    if args.dset__split_type > 5 or args.dset__split_type < 1:
        raise Exception("No such split type")
...
def get_random_device(args):
    ...
    """
    device_id = np.random.choice(3) + 1 # not use cuda:0 since it's always full
...
def get_dset_path(params):
    if params["dset"]["path"] is None and params['dset']['split_type'] == 0:
        params['dset']['train_path'] = f"{DATA_DIR}/{params['molecule']}_train_{params['dset']['split_type']:02d}.pth.tar"
    return params


def get_model_dir(params):
    if len(params["model"]["outdir"]) == 0 or params["model"]["outdir"] is None:
        params['model']['outdir'] = f"{MODEL_DIR}/{params['model']['uncertainty_type']}/e{params['loss']['energy_loss'][:3]}_f{params['loss']['forces_loss'][:3]}/{params['molecule']}/{params['id']}"
    return params
...
def two_params_equal(params1, params2):
    for key in params1.keys():
        if key in ['id']:
            continue
        if not isinstance(params1[key], dict):
            ...
            for k in params1[key].keys():
                ...
                if (key == 'dset') and (k == 'path'):
                    continue
                if (key == 'dset') and (k in ['train_inds', 'val_inds', 'test_inds']):
                    continue
...
if __name__ == "__main__":
    ...
    check_duplicated_params(params, force=args.force)
